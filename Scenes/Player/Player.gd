...
onready var hungerbar = $ViewportContainer/GUI/Hunger
onready var timer = $ViewportContainer/GUI/HungerTimer
...
func _process(delta):
	...
	if hunger <= 0:
		hunger = 0
		print("Hunger is at 0")
		#$ViewportContainer/GUI/HungerSystem/Timer.stop()
...
func update_healthbar(health):
	if $ViewportContainer/GUI/Health.value <= 0:
		print("YOU DIED!")
		SceneChanger.change_scene("res://Scenes/Death.tscn", "fade")
	$ViewportContainer/GUI/Health.set_value(health)
